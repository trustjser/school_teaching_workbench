...
| `active` | 进行中 | 是（默认值） |
| `closed` | 已结束 | 是 |
| `archived` | 已归档 | 否（仅历史数据兼容） |

- 界面只暴露「进行中 ⇄ 已结束」两态，对应命令 `task_set_status(taskId, status)`。
- `task_set_status` **只更新 `status` 一列**，并把 `sync_state` 置回 `pending`、`dirty` 置 1，使状态变更能随任务同步。禁止用 `task_upsert`（整行覆盖）改状态。
- 非法取值返回 `ERR_VALIDATION`；任务不存在或已软删返回 `ERR_NOT_FOUND`。
- 「已结束」**不冻结**学生记录：结束只是标签，之后仍可继续标记；可随时「重新开始」。
- 班级端对**所有**任务（含教务下发）都可标记状态 —— 教务端不消费该字段，且「我们班做完了没有」属于班级侧事实判断。这与「状态节点定义归教务端、班级端只读」是两件事。
