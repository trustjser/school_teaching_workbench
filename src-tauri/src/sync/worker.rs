...
use crate::config::constants::{Events, QUEUE_BATCH_SIZE, SYNC_IDLE_INTERVAL_MS, SYNC_POLL_INTERVAL_MS};
...
async fn run_once(state: &Arc<AppState>) {
    ...
    for item in pending {
        ...
        match deliver_item(state, &item).await {
            Ok(http_status) => {
                ...
                sync_repo::insert(
                    &state.pool, "out", Some(&item.target_device_id.clone().unwrap_or_default()),
                    None, Some(&item.target_endpoint), Some(&item.entity_type), 1, "success",
                    Some(http_status as i64), None, None, Some(dur), Some(&item.id), None,
                ).await.ok();
                sent += 1;
            }
            Err((code, msg)) => {
                ...
                let dead = queue_repo::mark_failed(
                    &state.pool, &item.id, &code.as_str(),
                    backoff::next_retry_at(item.attempt_count + 1),
                ).await.unwrap_or(false);
                sync_repo::insert(
                    &state.pool, "out", item.target_device_id.as_deref(), None, Some(&item.target_endpoint),
                    Some(&item.entity_type), 1, if dead { "dead" } else { "failed" }, None,
                    Some(&code.as_str()), Some(&msg), Some(dur), Some(&item.id), None,
                ).await.ok();
                if dead {
                    let _ = state.app.emit(Events::SYNC_ERROR, serde_json::json!({
    ...
    if sent + failed > 0 {
        let _ = state.app.emit(Events::SYNC_QUEUE_CHANGED, serde_json::json!({
...
async fn deliver_item(
    ...
    match item.op_type.as_str() {
        ...
        "ack" => {
            let to = item.target_device_id.clone().ok_or((ErrorCode::Validation, "回执缺少目标设备".into()))?;
            let base_url = item.target_base_url.clone().ok_or((ErrorCode::Net, "回执缺少目标地址".into()))?;
            let payload: Value = serde_json::from_str(&item.payload).map_err(|e| (ErrorCode::Validation, format!("回执载荷非法: {}", e)))?;
            let status = client::deliver(state, &to, &base_url, &item.target_endpoint, "POST", &payload)
                .await
                .map_err(|e| (e.code, e.message))?;
            Ok(status)
...
async fn deliver_ingest(
    ...
    if let (Some(to), Some(base_url)) = (&item.target_device_id, &item.target_base_url) {
        ...
        let status = client::deliver(state, to, base_url, &item.target_endpoint, "POST", &serde_json::to_value(&req).map_err(|e| (ErrorCode::Validation, e.to_string()))?)
            .await
            .map_err(|e| (e.code, e.message))?;
        return Ok(status);
    }
    ...
    let online: Vec<_> = masters
        ...
        .filter_map(|d| match (d.ip_address, d.port) {
            (Some(ip), Some(port)) if port > 0 => Some((d.device_id, format!("http://{}:{}", ip, port))),
            _ => None,
        })
        .collect();
    ...
    let req = build_ingest_request(state, &ingest_item).await;
    let req_value = serde_json::to_value(&req).map_err(|e| (ErrorCode::Validation, e.to_string()))?;
    let mut last_err: Option<(ErrorCode, String)> = None;
    let mut ok_count = 0u32;
    for (to, base_url) in online {
        match client::deliver(state, &to, &base_url, &item.target_endpoint, "POST", &req_value).await {
            Ok(_) => ok_count += 1,
...
pub async fn flush(state: &Arc<AppState>) -> Result<crate::db::models::FlushReport, (ErrorCode, String)> {
    let mut sent = 0i64;
    let mut failed = 0i64;
    loop {
        ...
        for item in pending {
            ...
            match deliver_item(state, &item).await {
                ...
                Err((code, _msg)) => {
                    queue_repo::mark_failed(&state.pool, &item.id, &code.as_str(), backoff::next_retry_at(item.attempt_count + 1)).await.ok();
                    failed += 1;
                }
            }
        }
    }
    let remaining = queue_repo::count_by_status(&state.pool, "pending").await.unwrap_or(0)
        + queue_repo::count_by_status(&state.pool, "sending").await.unwrap_or(0);
    Ok(crate::db::models::FlushReport { sent, failed, remaining })
}
